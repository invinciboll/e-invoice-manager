...
export const invoiceTypeMappings = new Map<InvoiceCode, string>([
...
export const useInvoiceTypeTranslator = () => {
    ...
    const translateInvoiceType = (code: number): string => {
        const key = invoiceTypeMappings.get(code as InvoiceCode) ?? "invoiceTypes.unknown";
        return t(key);
    };

    /**
     * Retrieves the description of an invoice type.
     * @param code The invoice type code (e.g., 326, 380).
     * @returns The translated description in the current language.
     */
