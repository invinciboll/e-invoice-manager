...
export const GraphsPanel: React.FC<GraphsPanelProps> = ({ invoices }) => {
    ...
    const sumPerMonth = invoices.reduce((acc, invoice) => {
        const month = new Date(invoice.issuedDate).toLocaleString("default", {
            month: "long",
        });
    ...
    const barData = Object.entries(sumPerMonth).map(([month, sum]) => ({
        month,
        amount: sum,
    }));
    ...
    const pieData = Object.entries(fileFormatDistribution).map(
        ([format, count], index) => ({
            format,
            count,
